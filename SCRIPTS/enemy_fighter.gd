...
func shoot():
	randomize()
	shoot_interval = randf_range(1., 4.0) # 5-20
	shoot_timer = 0
...
func die():
	...
	if Global.set_level_difficulty == 1:
		MissionsBattlepass.killed_nautolanFighter += 1
		MissionsBattlepass.on_enemy_killed("Fighter", 1)
	elif Global.set_level_difficulty == 2:
		MissionsBattlepass.killed_klaedFighter += 1
		MissionsBattlepass.on_enemy_killed("Fighter", 2)
	else: 
		MissionsBattlepass.killed_nairanFighter += 1
		MissionsBattlepass.on_enemy_killed("Fighter", 3)
